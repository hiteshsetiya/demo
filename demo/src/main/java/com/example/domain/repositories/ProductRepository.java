...
public interface ProductRepository extends IModelRepo, JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    static Specification<Product> colorContains(String color) {
        return (product, cq, cb) -> cb.like(product.get("color"), "%" + color + "%");
    }

    static Specification<Product> categoriesContains(Categories categories) {
        return (product, cq, cb) -> cb.like(product.get("categories"), "%" + categories + "%");
    }
